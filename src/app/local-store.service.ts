...
export class LocalStoreService {
  ...
  deleteLists(list:IList){
    ...
    if (text!=null){ this.listArray = JSON.parse(text); }
    let equaledList= this.listArray.filter((value: IList) => value.id !== list.id);
  ...
  deleteTasks(task:ITasks){
    ...
    if (text!=null){ this.taskArray = JSON.parse(text); }
    let equaledtask= this.taskArray.filter((value: ITasks) => value.id !== task.id);
   ...
   deleteListsTasks(listId:number){
    ...
    if (text!=null){ this.taskArray = JSON.parse(text); }
    let equaledtask= this.taskArray.filter((value: ITasks) => value.ListId !== listId);
    this.taskArray=equaledtask;
